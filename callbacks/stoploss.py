...
"""Add these functions to callbacks/stoploss.py for multi-strike stop-loss."""

async def handle_multi_stoploss(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Handle multi-strike stop-loss callback - display positions for multiple selection.
    ...
    try:
        ...
        if not positions:
            ...
            await query.edit_message_text(
                f"{EMOJI_SHIELD} <b>Multi-Strike Stop-Loss</b>\n\n"
                "You have no active positions.",
                reply_markup=reply_markup,
                parse_mode='HTML'
            )
        ...
        keyboard.append([InlineKeyboardButton(
            "✅ Confirm Selection",
            callback_data=create_callback_data(CALLBACK_MULTI_SL_TOGGLE, "confirm")
        )])
        keyboard.append([InlineKeyboardButton("🔙 Back to Main Menu", callback_data=CALLBACK_MAIN_MENU)])
        ...
        await query.edit_message_text(
            f"{EMOJI_SHIELD} <b>Multi-Strike Stop-Loss</b>\n\n"
            f"Select positions to set stop-loss (tap to toggle):\n"
        ...
        logger.error(f"Error fetching positions for multi stop-loss: {e}", exc_info=True)
...
async def handle_multi_stoploss_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Handle position toggle for multi-strike stop-loss.
    ...
    try:
        ...
        if action == "confirm":
            ...
            user_context.conversation_state = STATE_AWAITING_MULTI_SL_TRIGGER
            ...
            await query.edit_message_text(
                ...
                f"<i>Example: 5 (for 5% stop-loss)</i>",
                reply_markup=reply_markup,
                parse_mode='HTML'
            )
            return
        ...
        logger.error(f"Error in multi stop-loss toggle: {e}", exc_info=True)
        await query.answer("❌ An error occurred", show_alert=True)

async def handle_multi_stoploss_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Handle multi-strike stop-loss percentage input.
    ...
    if state not in [STATE_AWAITING_MULTI_SL_TRIGGER, STATE_AWAITING_MULTI_SL_LIMIT]:
        return
    ...
    try:
        ...
        if state == STATE_AWAITING_MULTI_SL_TRIGGER:
            user_context.temp_data['multi_sl_trigger_pct'] = value
            user_context.conversation_state = STATE_AWAITING_MULTI_SL_LIMIT
        ...
        elif state == STATE_AWAITING_MULTI_SL_LIMIT:
            user_context.temp_data['multi_sl_limit_pct'] = value
            user_context.conversation_state = None
            
            # Show confirmation
            await _show_multi_stoploss_confirmation(update, user_context)
    
    except Exception as e:
        logger.error(f"Error processing multi stop-loss input: {e}", exc_info=True)
        await update.message.reply_text(
            "❌ An error occurred. Please try again or use /start.",
            parse_mode='HTML'
        )

async def handle_multi_stoploss_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Handle multi-strike stop-loss confirmation - place orders for all selected positions.
    ...
    try:
        ...
        trigger_pct = user_context.temp_data['multi_sl_trigger_pct']
        limit_pct = user_context.temp_data['multi_sl_limit_pct']
        ...
        # Show executing message
        await query.edit_message_text(
            f"{EMOJI_SHIELD} <b>Placing Stop-Loss Orders...</b>\n\n"
        ...
        with DeltaClient(
            ...
            order_api = OrderAPI(client)
            results = order_api.place_batch_stop_orders(orders_data, order_type='stop_loss')
        ...
        result_text = (
            f"{EMOJI_CHECK} <b>Multi-Strike Stop-Loss Complete</b>\n\n"
        ...
        logger.info(f"User {user_id} placed multi-strike stop-loss: {successful}/{len(results)} successful")
    
    except Exception as e:
        logger.error(f"Error placing multi stop-loss orders: {e}", exc_info=True)
        
        keyboard = [[InlineKeyboardButton("🔙 Back to Main Menu", callback_data=CALLBACK_MAIN_MENU)]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        await query.edit_message_text(
            f"{EMOJI_CROSS} <b>Failed to Place Multi-Strike Stop-Loss</b>\n\n"
            f"Error: {str(e)}",
            reply_markup=reply_markup,
            parse_mode='HTML'
        )
...
async def _show_multi_stoploss_confirmation(update, user_context):
    """Show multi-strike stop-loss confirmation."""
    trigger_pct = user_context.temp_data['multi_sl_trigger_pct']
    limit_pct = user_context.temp_data['multi_sl_limit_pct']
    selected_count = len(user_context.temp_data['selected_positions'])
    
    confirmation_text = (
        f"{EMOJI_SHIELD} <b>Confirm Multi-Strike Stop-Loss</b>\n\n"
        f"<b>Positions Selected:</b> {selected_count}\n"
        f"<b>Trigger %:</b> {format_percentage(trigger_pct)}\n"
        f"<b>Limit %:</b> {format_percentage(limit_pct)}\n\n"
        f"This will place stop-loss orders for all selected positions.\n\n"
        f"Confirm to proceed?"
    )
    
    keyboard = [
        [InlineKeyboardButton(f"{EMOJI_CHECK} Confirm", callback_data=CALLBACK_CONFIRM_MULTI_SL)],
        [InlineKeyboardButton(f"{EMOJI_CROSS} Cancel", callback_data=CALLBACK_MAIN_MENU)]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
